...
class MainClass(object):
    ...
    def resource_path(self, relative_path):
        ...
        try:
            base_path = sys._MEIPASS  # quando rodando como .exe
        except AttributeError:
            base_path = os.path.abspath(".")  # quando rodando como .py
    ...
    def filter_c(self, contours_c):
        ...
        c_copy = [cnt.copy() for cnt in contours_c]
    ...
    def zoom_cut(self, event):
        if self.lBrilho.cget("image") == "":
            ...
            rel_x_in_widget = x / widget_width
            rel_y_in_widget = y / widget_height

            # converter para posição absoluta na imagem original
            self.relative_x = (self.x0 + rel_x_in_widget * (self.x1 - self.x0)) / self.image_cv2.shape[1]
            self.relative_y = (self.y0 + rel_y_in_widget * (self.y1 - self.y0)) / self.image_cv2.shape[0]
    ...
    def scroll(self, event):
        if self.lBrilho.cget("image") == "":
            ...
            #posição relativa do cursor
            # posição do cursor relativa à imagem exibida
            rel_x_in_widget = x / widget_width
            rel_y_in_widget = y / widget_height
